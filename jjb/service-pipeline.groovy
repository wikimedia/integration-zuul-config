node('ServicePipelineK8s') {
  ...
  def testDeployment = {
    ...
    try {
      sh "KUBECONFIG=/etc/kubernetes/ci-staging.config helm install --namespace CI ${overrides} -n ${arg(release)} --debug --wait --timeout ${timeout} ${arg(config.chart)}"
      sh "KUBECONFIG=/etc/kubernetes/ci-staging.config helm test --cleanup ${arg(release)}"
    } finally {
      sh "KUBECONFIG=/etc/kubernetes/ci-staging.config helm delete --purge ${arg(release)}"
    }
  }
