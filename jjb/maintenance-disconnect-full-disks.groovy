...
def removeDockerImages = { computer ->
    // The 10000 limit on docker rmi arguments to send is very conservative
    // based on the 2097152 ARG_MAX of our slave instances and the typical
...
def checkAgents = {
    ...
    for (slave in hudson.model.Hudson.instance.slaves) {
        ...
        diskObjects.each{ diskObject ->
            ...
            // If the cleanup threshold is reached but not the greater offline
            // threshold, attempt to remove old docker images
            if (diskObject.percent < cleanupPercentage) {
                return
            } else if (diskObject.percent < offlinePercentage && diskObject.mount == dockerMount) {
