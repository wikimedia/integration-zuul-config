...
cd $MW_DIR
git fetch --quiet --depth 1 "$ZUUL_URL/mediawiki/core" master
git checkout FETCH_HEAD
cd $MW_DIR/vendor
git fetch --quiet --depth 1 "$ZUUL_URL/mediawiki/vendor" master
git checkout FETCH_HEAD
cd $MW_DIR
...
do
    ...
    cd $DEP_DIR
    git init
    git fetch --quiet --depth 1 "${ZUUL_URL}/${DEP}" master
    git checkout FETCH_HEAD
...
cd $SRC_DIR
git init
git fetch --quiet --depth 1 --quiet "${ZUUL_URL}/${ZUUL_PROJECT}" "$ZUUL_REF"
