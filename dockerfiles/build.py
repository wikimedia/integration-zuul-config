...
class DockerBuilder(object):
    ...
    def run(self):
        ...
        for image in to_build:
            ...
            if recurse:
                ...
                # Extend build list to sub-images. Note that we need to strip the "wmfreleng/" prefix
                images.extend([x.split('/')[1] for x in self.gen_staged_deps(tree)])
    ...
    def gen_deps_tree(self, name):
        ...
        for f in self.find_docker_files():
            image_name = DOCKER_HUB_ACCOUNT + '/' + os.path.basename(os.path.dirname(f))
            with open(f) as fp:
                froms[image_name] = []
