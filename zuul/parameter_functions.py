...
def set_parameters(item, job, params):
    ...
    if job.name.startswith('release-quibble-'):
        set_release_repos(item, job, params)
...
def set_release_repos(item, job, params):
    exts = [e for e in release_repos
            if e.startswith('mediawiki/extensions/')]
    exts = sorted(list(set(exts)))
    params['EXT_DEPENDENCIES'] = '\\n'.join(exts)

    skins = [e for e in release_repos
